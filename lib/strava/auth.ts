...
function getEncryptionKey() {
    const secret =
        getRequiredEnv("STRAVA_SESSION_SECRET");
...
export async function decryptStravaAuth(token: string): Promise<StravaAuth | null> {
    try {
...
export function isAccessTokenExpired(expiresAt: number): boolean {
    ...
    /*
     * Refresh when the token has
     * one hour or less remaining.
     *
     * Strava recommends checking
     * expiration and refreshing when
     * needed.
     */
...
export async function refreshStravaAuth(
    ...
    if (!response.ok) {
        console.error("Strava refresh failed:",
            {
